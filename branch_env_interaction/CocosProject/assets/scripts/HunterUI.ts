...
import { HunterController, BoardBreakPhase } from './HunterController';
...
export class HunterUI extends Component {
    ...
    private _currentBreakBoard: Board | null = null;
    ...
    start() {
        ...
        this.hunterController.onBoardBreakProgressUpdate((board, progress, phase) => {
            this.onBoardBreakProgress(board, progress, phase);
        });
    ...
    update(deltaTime: number) {
        ...
        if (this.breakBoardButton && this.breakBoardButton.active) {
            this.updateBreakBoardButton();
        }
    }
    ...
    private onBoardTargetChange(board: Board | null) {
        this._currentBreakBoard = board;
        if (board || (this.hunterController && this.hunterController.isBreakingBoard())) {
    ...
    private hideBreakBoardButton() {
        ...
        this._currentBreakBoard = null;
    }

    private updateBreakBoardButton() {
        if (!this.breakBoardButton || !this.breakBoardButton.active || !this.hunterController) {
            return;
        }

        const button = this.breakBoardButton.getComponent(Button);
        if (button) {
            button.interactable = this.hunterController.canBreakBoard();
        }
        ...
        const isBreaking = this.hunterController.isBreakingBoard();
        if (isBreaking) {
            const progress = Math.floor(this.hunterController.getBoardBreakProgress() * 100);
            this.breakBoardLabel.string = `取消踩碎 (${progress}%)`;
        } else {
            this.breakBoardLabel.string = "踩碎木板";
        }
    }

    private onBoardBreakProgress(board: Board | null, progress: number, phase: BoardBreakPhase) {
        this._currentBreakBoard = board;
        ...
        if (!this.breakBoardButton.active) {
            this.breakBoardButton.active = true;
        }
