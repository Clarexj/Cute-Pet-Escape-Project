    ...
    execute: function () {
      ...
      _export("HangTimerUI", HangTimerUI = (_dec = ccclass('HangTimerUI'), _dec2 = property(Label), _dec3 = property(Sprite), _dec4 = property(Label), _dec5 = property(Node), _dec(_class = (_class2 = class HangTimerUI extends Component {
        ...
        update(deltaTime) {
          ...
          var isHanged = this._characterState.isHanged(); // 控制显示/隐藏
        ...
        updateTimerDisplay() {
          ...
          var timeRemaining = this._characterState.getHangTimeRemaining();

          var seconds = Math.ceil(timeRemaining); // 更新文字

          this.timerLabel.string = seconds + "s"; // 更新进度条（可选）

          if (this.timerBar) {
            var progress = timeRemaining / this._characterState.hangDuration; // 通过fillRange控制进度条
        ...
        updateLivesDisplay() {
          ...
          var currentLives = this._characterState.getRemainingLives();

          var maxLives = this._characterState.maxHangCount;
        ...
        updatePosition() {
          ...
          var charWorldPos = this.targetCharacter.getWorldPosition(); // 设置UI位置在角色上方
