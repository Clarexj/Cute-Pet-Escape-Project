    ...
    execute: function () {
      ...
      _export("PlayerSyncController", PlayerSyncController = (_dec = ccclass('PlayerSyncController'), _dec(_class = (_class2 = class PlayerSyncController extends Component {
        ...
        syncPlayerState() {
          ...
          var currentRotationY = this.node.eulerAngles.y; // 检测是否正在移动（通过位置变化判断）

          var positionChanged = Vec3.distance(this._tempPosition, this._lastPosition) > this.positionThreshold;
          var rotationChanged = Math.abs(currentRotationY - this._lastRotationY) > this.rotationThreshold; // 获取动画状态（如果有CharacterAnimationController）

          var animationState = "idle";
          var isMoving = false; // 尝试从PlayerController或其他移动控制器获取移动状态

          var playerController = this.node.getComponent('PlayerController');
          ...
          var stateChanged = positionChanged || rotationChanged || isMoving !== this._lastIsMoving || animationState !== this._lastAnimationState; // 只在状态变化时发送（节省带宽）
        ...
        forceSyncState() {
          ...
          this.node.getPosition(this._tempPosition);
          var currentRotationY = this.node.eulerAngles.y; // 获取移动状态
