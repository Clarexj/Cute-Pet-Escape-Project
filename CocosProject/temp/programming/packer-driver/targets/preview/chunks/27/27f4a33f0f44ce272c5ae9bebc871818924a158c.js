    ...
    execute: function () {
      ...
      _export("Board", Board = (_dec = ccclass('Board'), _dec(_class = (_class2 = class Board extends (_crd && Interactable === void 0 ? (_reportPossibleCrUseOfInteractable({
        ...
        onInteract(player) {
          ...
          var playerController = player.getComponent(_crd && PlayerController === void 0 ? (_reportPossibleCrUseOfPlayerController({
            error: Error()
        ...
        fallDown(playerController) {
          ...
          var fallDir = this.fallDirection.clone().normalize(); // 计算倒下的目标旋转（绕X轴或Z轴旋转90度）
          // 这里简化处理：假设木板绕X轴旋转

          var targetRotation = new Quat(); // 根据倒下方向计算旋转轴
          // 如果倒向+Z方向，绕X轴正向旋转90度
          // 如果倒向-Z方向，绕X轴负向旋转90度
          // 如果倒向+X方向，绕Z轴负向旋转90度
          // 如果倒向-X方向，绕Z轴正向旋转90度
          ...
          if (Math.abs(fallDir.z) > Math.abs(fallDir.x)) {
            ...
            var angle = fallDir.z > 0 ? 90 : -90;
            Quat.fromAxisAngle(targetRotation, rotationAxis, angle * Math.PI / 180);
          } else {
            // 主要沿X轴倒下，绕Z轴旋转
            rotationAxis.set(0, 0, 1);

            var _angle = fallDir.x > 0 ? -90 : 90;

            Quat.fromAxisAngle(targetRotation, rotationAxis, _angle * Math.PI / 180);
          } // 组合初始旋转和倒下旋转


          Quat.multiply(targetRotation, this._initialRotation, targetRotation); // 使用tween动画实现平滑旋转

          var startRotation = this.node.rotation.clone();
