    ...
    execute: function () {
      ...
      _export("Ore", Ore = (_dec = ccclass('Ore'), _dec(_class = (_class2 = class Ore extends (_crd && Interactable === void 0 ? (_reportPossibleCrUseOfInteractable({
        ...
        collect() {
          ...
          var initialPosition = this.node.position.clone();
          var initialScale = this.node.scale.clone(); // 目标位置（向上漂浮）

          var targetPosition = new Vec3(initialPosition.x, initialPosition.y + this.floatUpDistance, initialPosition.z); // 目标缩放（放大后缩小到0）

          var enlargeScale = new Vec3(initialScale.x * this.collectScaleUp, initialScale.y * this.collectScaleUp, initialScale.z * this.collectScaleUp); // 使用tween实现拾取动画
