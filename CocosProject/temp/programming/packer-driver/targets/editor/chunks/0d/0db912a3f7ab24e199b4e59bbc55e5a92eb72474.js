    ...
    execute: function () {
      ...
      _export("Hunter", Hunter = (_dec = ccclass('Hunter'), _dec(_class = (_class2 = class Hunter extends Component {
        ...
        onTriggerEnter(event) {
          const otherNode = event.otherCollider.node; // 检查是否是逃生者

          const characterState = otherNode.getComponent(_crd && CharacterState === void 0 ? (_reportPossibleCrUseOfCharacterState({
            error: Error()
        ...
        onTriggerStay(event) {
          const otherNode = event.otherCollider.node; // 检查是否碰到正在救援的逃生者，打断救援
        ...
        onTriggerExit(event) {
          const otherNode = event.otherCollider.node; // 离开笼子范围
        ...
        autoDetectSurvivors() {
          // 查找所有逃生者
          const allSurvivors = this.node.scene.getComponentsInChildren(_crd && CharacterState === void 0 ? (_reportPossibleCrUseOfCharacterState({
            error: Error()
          }), CharacterState) : CharacterState);
        ...
        hangSurvivorOnCage(cageNode) {
          ...
          const characterState = this._carriedSurvivor.getComponent(_crd && CharacterState === void 0 ? (_reportPossibleCrUseOfCharacterState({
            error: Error()
          }), CharacterState) : CharacterState);
