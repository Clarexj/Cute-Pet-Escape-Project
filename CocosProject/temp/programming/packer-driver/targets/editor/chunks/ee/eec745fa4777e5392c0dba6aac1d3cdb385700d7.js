    ...
    execute: function () {
      ...
      _export("UIManager", UIManager = (_dec = ccclass('UIManager'), _dec2 = property(Label), _dec3 = property(Label), _dec4 = property(Node), _dec5 = property([Sprite]), _dec(_class = (_class2 = class UIManager extends Component {
        ...
        initializeTeamMemberUI() {
          ...
          const allCharacters = this.node.scene.getComponentsInChildren(_crd && CharacterState === void 0 ? (_reportPossibleCrUseOfCharacterState({
            error: Error()
          }), CharacterState) : CharacterState); // 筛选出逃生者（排除追捕者）
          // 假设追捕者节点名包含"Hunter"，逃生者节点名包含"Survivor"

          const survivors = allCharacters.filter(char => char.node.name.toLowerCase().includes('survivor'));
          ...
          for (let i = 0; i < maxMembers; i++) {
            ...
            this._teamMembers.push(memberData); // 注册状态变化回调
        ...
        updateTimerDisplay() {
          ...
          const seconds = Math.floor(timeRemaining % 60); // 格式化为 MM:SS
