    ...
    execute: function () {
      ...
      _export("PlayerController", PlayerController = (_dec = ccclass('PlayerController'), _dec2 = property(_crd && Joystick === void 0 ? (_reportPossibleCrUseOfJoystick({
        ...
        detectInteractables() {
          ...
          let closestDistance = this.interactDistance; // 获取玩家位置

          this.node.getWorldPosition(this._rayOrigin); // ✅ 优化：使用缓存的列表而非每帧遍历场景树

          for (const interactable of this._allInteractables) {
            if (!interactable.canInteract()) continue; // 获取可交互物体位置

            interactable.node.getWorldPosition(this._tempVec3_1); // 计算距离

            Vec3.subtract(this._tempVec3_2, this._tempVec3_1, this._rayOrigin);

            const distance = this._tempVec3_2.length(); // 检查是否在交互范围内
        ...
        triggerRescue() {
          ...
          const targetState = this._currentRescueTarget.getComponent(_crd && CharacterState === void 0 ? (_reportPossibleCrUseOfCharacterState({
            error: Error()
          }), CharacterState) : CharacterState);
